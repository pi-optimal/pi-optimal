...
class ModelBasedEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if state_input_transfrom.shape[1] < self.dataset.forecast_timesteps:
            pad = np.zeros((1, self.dataset.forecast_timesteps - state_input_transfrom.shape[1], state_input_transfrom.shape[2]))
            state_input_transfrom = np.concatenate([pad, state_input_transfrom], axis=1)

        if self.action.shape[1] < self.dataset.forecast_timesteps:
            pad = np.zeros((1, self.dataset.forecast_timesteps - self.action.shape[1], self.action.shape[2]))
        ...
        if self.state.shape[1] < self.dataset.forecast_timesteps:
